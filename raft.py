...
class RaftNode(Actor):
    ...
    def advice_peers(self):
        senders = self.peers
        for peer in senders:
            print(f"mandando heartbeat para o peer {peer}")
            self.network.send(peer, HeartbeatUpdate(self.state.phase != RaftState.PHASE1))

    def advice_leader(self):
        senders = self.leader_id
        print(f"mandando aviso para o líder")
        self.network.send(senders, SuspensionNotify(self.state.phase != RaftState.PHASE1 and RaftState.PHASE2))


    def handle_suspension_notify(self, req):
        """
        TODO Líder identificando o nó que enviou a mensagem e excluindo ele da lista de peers para mandar heartbeat/entradas

        """

    def proxy_advice_leader(self):
        senders = self.leader_id
        print(f"mandando aviso para o líder")
        self.network.send(senders, ProxyShutdown(self.state.phase != RaftState.PHASE1 and RaftState.PHASE2 and RaftState.PHASE3))


    def handle_proxy_advice(self, req):
        print("Escolhendo novo proxy")
        proxy_shutdown = req.sender

        if proxy_shutdown in self.state.proxy_candidates:
            del self.state.proxy_candidates[proxy_shutdown]
        ...
        match role:
            case RaftState.LEADER:
                ...
                if self.state.phase == RaftState.PHASE2:    
                    ...
                    print(f"Incrementando o meu intervalo de heartbeats em: {SLOWHEARTBEAT}")
                    self.heartbeat_interval += SLOWHEARTBEAT
                    self.advice_peers()
                ...
                if self.state.phase == RaftState.PHASE4:
                    ...
                    print("Retornando para o estado de seguidor...")
                    self.demote()
            ...
            case RaftState.PROXY:
                ...
                if self.state.phase == RaftState.PHASE4:
                    ...
                    print("Avisando ao líder do desligamento...")
                    self.proxy_advice_leader()
            ...
            case RaftState.FOLLOWER:
                ...
                if self.state.phase == RaftState.PHASE3:
                    ...
                    print("Entrando em modo de suspensão")
                    self.state.become_suspendedfollower()
                    self.advice_leader()
